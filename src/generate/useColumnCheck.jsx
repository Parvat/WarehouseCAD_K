...
const EMPTY_RESULT = {
  ...
  redMarks:      [],
  summary: {
    profile: '', rackConflicts: 0, blockedAisles: 0,
...
export function ColumnCheckProvider({ children }) {
  ...
  const result = useMemo(() => {
    ...
    /* A hidden grid is a grid that is switched off — ColumnGridPanel's
       ...
       of the recalc triggers the brief asks for. */
    ...
    return checkColumns({ racks, columns, profile, gridSize })
  }, [objects, gridSize, profile])
  ...
  const value = useMemo(() => ({
    result, profile, mheKey, setMheKey,
    showMarks, setShowMarks,
    resolutions, setResolution,
  }), [result, profile, mheKey, showMarks, resolutions, setResolution])
